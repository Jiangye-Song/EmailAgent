...
### Phase 5.1 — Inbox Actions & Rich AI Rendering (Day 4–5)

**Goal:** Replace the current approve/reject flow with useful mailbox actions and render AI-generated sections in markdown with actionable buttons.

- [ ] **Replace Approve / Reject with email-native actions** across inbox detail and list interactions:
  - [ ] **Star / Unstar** an email
  - [ ] **Mark as unread** after opening an email (restores unread state and updates badge counts)
  - [ ] **Remove** an email and all related derived data stored for it

- [ ] **Add read-state tracking** to `email_records`:
  - [ ] Persist `is_read` state separately from selection/open state
  - [ ] Opening an email marks it as read
  - [ ] `Mark as unread` sets `is_read = false` and restores it to unread counts
  - [ ] Category/sidebar badges should count unread emails where applicable

- [ ] **Add starring support** to `email_records` and inbox navigation:
  - [ ] Persist `is_starred` state
  - [ ] Add a **Starred** filter/list in the left sidebar
  - [ ] Allow toggling star state from both email list rows and email detail header

- [ ] **Implement destructive remove flow**:
  - [ ] Add a server action for permanent deletion of an email
  - [ ] Delete the `email_records` row and any related stored artifacts for that email
  - [ ] Ensure summary, action items, embeddings, and any email-specific digest/export data are removed with it
  - [ ] Require a confirmation step in the UI before destructive deletion

- [ ] **Refactor inbox actions layer** (`src/lib/actions/email-actions.ts`):
  - [ ] Remove `approveAction()` / `rejectAction()` as the primary UI actions for inbox triage
  - [ ] Add `toggleStarEmail(emailId)`
  - [ ] Add `markEmailUnread(emailId)`
  - [ ] Add `removeEmail(emailId)`
  - [ ] Revalidate `/inbox` after each mutation

- [ ] **Update inbox UI components** for the new action model:
  - [ ] `src/components/inbox/EmailDetail.tsx` — replace approve/reject controls with star, mark unread, remove, and reply-related actions
  - [ ] `src/components/inbox/EmailList.tsx` — show unread/starred state visually and support quick star toggle
  - [ ] `src/components/inbox/InboxSidebar.tsx` — add Starred entry and unread-aware counts
  - [ ] `src/components/inbox/InboxLayout.tsx` — support starred filter alongside category filters

- [ ] **Render AI-generated sections as markdown** instead of plain text where applicable:
  - [ ] Summary
  - [ ] Action items
  - [ ] Any future structured sections generated by the agent
  - [ ] Preserve safe markdown rendering and consistent typography in the reading pane

- [ ] **Support actionable buttons inside AI-generated sections**:
  - [ ] For action items, allow the agent to return markdown plus structured button metadata
  - [ ] Buttons may be:
    - [ ] External/custom CTA buttons derived from URLs found in the email
    - [ ] Email-native actions such as star, remove, or reply
  - [ ] If the model can infer intent, allow it to suggest button label and visual tone/color
  - [ ] Keep rendering deterministic: parse structured button data rather than trusting arbitrary raw HTML from model output

- [ ] **Update AI output contract** in `src/lib/ai/processor.ts`:
  - [ ] Summary should support markdown output
  - [ ] Action items should support markdown output
  - [ ] Add optional structured action/button objects, for example:
    ```ts
    {
      label: string;
    ...
    ```
  - [ ] Store enough structured data in the DB to render buttons without reparsing raw model text on every request

- [ ] **Migration / data model updates**:
  - [ ] Add `is_read boolean default false` to `email_records`
  - [ ] Add `is_starred boolean default false` to `email_records`
  - [ ] Add structured markdown/button fields only if needed beyond existing summary/todo columns
